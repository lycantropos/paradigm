...
_CACHE_PATH = _Path(__file__).with_name(
    ...
    + '_'
    + _Path(__file__).with_suffix(_caching.FILE_SUFFIX).name
)
...
ObjectRawAstNodes = list[_serialization.RawAstNode]
...
superclasses: dict[_catalog.Path, _scoping.ModuleSuperclasses]
...
try:
    (
        ...
        _DEFINITIONS_FIELD_NAME,
        _NODES_KINDS_FIELD_NAME,
        _RAW_AST_NODES_FIELD_NAME,
        _REFERENCES_FIELD_NAME,
        _SUBMODULES_FIELD_NAME,
        _SUPERCLASSES_FIELD_NAME,
        _VERSION_FIELD_NAME,
        path=_CACHE_PATH,
    )
except Exception:
...
if _reload_cache:
    ...
    _ModuleSubmodules = list[_catalog.Path]
    _ModuleSuperclasses = dict[_catalog.Path, list[_catalog.QualifiedPath]]
    _Scope = dict[str, '_Scope']

    def _named_tuple_to_constructor_ast_node(
        ast_node: _ast.ClassDef,
    ) -> _ast.FunctionDef:
        annotations_ast_nodes: list[_ast.AnnAssign] = [
            ast_child_node
            for ast_child_node in ast_node.body
            if isinstance(ast_child_node, _ast.AnnAssign)
        ]
        assert all(
            isinstance(ast_node.target, _ast.Name)
    ...
    def _annotations_to_signature(
        ast_nodes: list[_ast.AnnAssign],
    ) -> _ast.arguments:
        return _ast.arguments(
            [],
            [_ast.arg('cls', None)]
            + [_ann_assign_to_arg(ast_node) for ast_node in ast_nodes],
            None,
            [],
            [],
            None,
    ...
    def _expression_to_assignment(
        node: _ast.expr, *, name: str
    ) -> _ast.Assign:
    ...
    class _StateParser(_ast.NodeVisitor):
        def __init__(
            ...
            modules_ast_nodes: dict[_catalog.Path, _ModuleAstNodes],
            modules_ast_nodes_kinds: dict[_catalog.Path, _ModuleAstNodesKinds],
            modules_definitions: dict[_catalog.Path, _Scope],
            ...
            visited_modules_paths: set[_catalog.Path],
            classes_bases: dict[_catalog.QualifiedPath, list[_ast.expr]],
            generics_parameters_paths: dict[
                _catalog.QualifiedPath, tuple[_catalog.Path, ...]
            ],
        ) -> None:
            (
                self.generics_parameters_paths,
                self.module_ast_nodes,
                self.module_ast_nodes_kinds,
                self.module_path,
                self.module_references,
                self.modules_ast_nodes,
                self.modules_ast_nodes_kinds,
        ...
        def visit_AnnAssign(self, node: _ast.AnnAssign) -> None:  # noqa: N802
        ...
        def visit_Assign(self, node: _ast.Assign) -> None:  # noqa: N802
        ...
        def visit_ClassDef(  # noqa: N802
            ...
            *,
            ...
            named_tuple_object_path: _catalog.Path = _catalog.path_from_string(  # noqa: B008
                _t.NamedTuple.__qualname__
            ),
            typing_module_path: _catalog.Path = (
                _catalog.module_path_from_module(_t)  # noqa: B008
            ),
            typing_extensions_module_path: _catalog.Path = (
                _catalog.module_path_from_module(_te)  # noqa: B008
            ),
        ) -> None:
                ...
                if _is_generic_specialization(base):
                    ...
                    type_vars_local_paths += [
                        ...
                        if (
                            ...
                            and self._is_type_var_object_path(
                                maybe_object_path
                            )
                        )
                    ]
                    ...
                    if (
                        base_origin_maybe_object_path is not None
                        ...
                        in [
                            ...
                            (
                                typing_extensions_module_path,
                                protocol_object_path,
                            ),
                        ]
                    ):
                        continue
            ...
            if generic_parameters:
                self.generics_parameters_paths[module_path, class_path] = (
                    generic_parameters
                )
        ...
        def visit_Import(self, node: _ast.Import) -> None:  # noqa: N802
        ...
        def visit_ImportFrom(self, node: _ast.ImportFrom) -> None:  # noqa: N802
        ...
        def _add_submodule(self, module_path: _catalog.Path) -> None:
            self.modules_submodules.setdefault(self.module_path, []).append(
                module_path
            )
        ...
        def _add_superclass(
            ...
            base_object_path: _catalog.Path,
        ) -> None:
            self.modules_superclasses.setdefault(
                self.module_path, {}
            ).setdefault(child_path, []).append(
                (base_module_path, base_object_path)
            )
    ...
    def _resolve_object_path(
        ...
        *,
        ...
        if _scoping.scope_contains_path(
            module_definitions, (*parent_path, first_name)
        ):
    ...
    @_singledispatch
    def _unpack_ast_node(ast_node: _ast.expr) -> tuple[_ast.expr, ...]:
        return (ast_node,)

    @_unpack_ast_node.register(_ast.Tuple)
    def _(ast_node: _ast.Tuple) -> tuple[_ast.expr, ...]:
        assert isinstance(ast_node.ctx, _ast.Load), ast_node
        return tuple(ast_node.elts)

    def _collect_type_args(ast_node: _ast.expr) -> list[_ast.expr]:
    ...
    def _parse_stubs_state(
        ...
        dict[_catalog.Path, _Scope],
        dict[_catalog.Path, _ModuleAstNodesKinds],
        dict[_catalog.Path, _ModuleRawAstNodes],
        dict[_catalog.Path, _ModuleReferences],
        dict[_catalog.Path, _ModuleSubmodules],
        dict[_catalog.Path, _ModuleSuperclasses],
    ]:
        modules_definitions: dict[_catalog.Path, _Scope] = {}
        modules_nodes_kinds: dict[_catalog.Path, _ModuleAstNodesKinds] = {}
        modules_ast_nodes: dict[_catalog.Path, _ModuleAstNodes] = {}
        modules_references: dict[_catalog.Path, _ModuleReferences] = {}
        modules_submodules: dict[_catalog.Path, _ModuleSubmodules] = {}
        modules_superclasses: dict[_catalog.Path, _ModuleSuperclasses] = {}
        ...
        visited_modules_paths: set[_catalog.Path] = set()
        _parse_stub_scope(
        ...
        modules_raw_ast_nodes = {
            module_path: {
                object_path: [
                    _serialization.to_raw(ast_node) for ast_node in ast_nodes
                ]
    ...
    def _process_classes_bases(
        ...
        *,
        ...
        for (
            class_module_path,
            class_object_path,
        ), class_bases in classes_bases.items():
            for base in class_bases:
                ...
                modules_superclasses.setdefault(
                    class_module_path, {}
                ).setdefault(class_object_path, []).append(
                    (base_module_path, base_object_path)
                )
        ...
        if _is_generic_specialization(base):
            ...
            if base_name not in specializations_module_scope:
                ...
                if (
                    generic_module_path,
                    generic_object_path,
                ) not in generics_parameters_paths:
                    pass
    ...
    def _register_generic_specialization(
        ...
        modules_superclasses: _t.Mapping[_catalog.Path, _ModuleSuperclasses],
        ...
        specializations_nodes_kinds[specialization_object_path] = (
            _NodeKind.CLASS
        )
