...
def _load_and_add(set_: set[Any], module_name: str, object_name: str) -> None:
    ...
    try:
        ...
        warnings.warn(
            f'Module "{module_name}" has no object with path "{path}".',
            stacklevel=2,
        )
    else:
...
def _load_and_update(
    set_: set[Any], module_name: str, names: Iterable[str]
) -> None:
    for name in names:
...
def _safe_import(name: str) -> types.ModuleType | None:
    try:
        ...
        warnings.warn(
            f'Failed to import module "{name}".', ImportWarning, stacklevel=2
        )
        return None
...
if sys.platform == 'linux':
    if sys.implementation.name == 'pypy':
        ...
        _load_and_update(
            classes,
            '_collections',
            ['deque_iterator', 'deque_reverse_iterator'],
        )
        _load_and_add(classes, '_cppyy', 'CPPInstance')
        _load_and_update(
            classes, '_ctypes.basics', ['_CDataMeta', 'bufferable']
        )
        _load_and_add(classes, '_ctypes.function', 'CFuncPtrType')
        _load_and_update(classes, '_ffi', ['CDLL', 'Field'])
        _load_and_update(
            classes,
            '_io',
        ...
        _load_and_update(
            classes,
            '_multibytecodec',
            ['MultibyteIncrementalDecoder', 'MultibyteIncrementalEncoder'],
        )
        ...
        _load_and_update(
            classes,
            'asyncio.events',
            ['_RunningLoop', 'BaseDefaultEventLoopPolicy._Local'],
        )
        _load_and_add(classes, 'builtins', 'NoneType')
        _load_and_update(
            classes,
            'cffi._pycparser.ply.yacc',
            ['GrammarError', 'LALRError', 'VersionError', 'YaccError'],
        )
        ...
        _load_and_add(
            classes, 'multiprocessing.process', 'AuthenticationString'
        )
        _load_and_add(classes, 'pickle', 'BytesBuilder')
        _load_and_add(
            classes,
            'pypy_tools.build_cffi_imports',
        ...
        _load_and_update(
            classes, 'stackless', ['CoroutineExit', 'TaskletExit']
        )
        _load_and_update(
            classes,
            'tarfile',
        ...
        _load_and_update(
            classes, 'unittest.case', ['_ShouldStop', '_UnexpectedSuccess']
        )
        ...
        _load_and_update(
            built_in_functions,
            '_thread',
            ['allocate', 'exit_thread', 'start_new'],
        )
        _load_and_add(
            built_in_functions, 'collections', 'UserString.maketrans'
        )
        ...
        _load_and_update(
            built_in_functions, 'heapq', ['_heappop_max', '_heapreplace_max']
        )
        _load_and_update(
            built_in_functions, 'json.decoder', ['c_scanstring', 'scanstring']
        )
        _load_and_update(
            built_in_functions,
            'json.encoder',
        ...
        _load_and_update(
            built_in_functions, 'locale', ['_localeconv', '_setlocale']
        )
        _load_and_add(built_in_functions, 'logging', 'Formatter.converter')
        _load_and_add(built_in_functions, 'math', 'hypot')
        _load_and_add(
            built_in_functions, 'multiprocessing.synchronize', 'sem_unlink'
        )
        ...
        _load_and_add(
            built_in_functions, 'selectors', 'PollSelector._selector_cls'
        )
        _load_and_update(
            built_in_functions, 'threading', ['_set_sentinel', 'excepthook']
        )
        ...
        _load_and_update(
            classes,
            '_collections',
            ['_deque_iterator', '_deque_reverse_iterator'],
        )
        _load_and_update(
            classes,
            '_io',
        ...
        _load_and_update(
            classes,
            'asyncio.events',
            ['BaseDefaultEventLoopPolicy._Local', '_RunningLoop'],
        )
        ...
        _load_and_add(classes, 'importlib._bootstrap', '_DeadlockError')
        _load_and_update(
            classes, 'itertools', ['_grouper', '_tee', '_tee_dataobject']
        )
        _load_and_add(classes, 'json.encoder', 'c_make_encoder')
        _load_and_update(
            classes, 'json.scanner', ['c_make_scanner', 'make_scanner']
        )
        _load_and_add(classes, 'lib2to3.patcomp', 'PatternSyntaxError')
        _load_and_add(classes, 'lib2to3.refactor', '_EveryNode')
        ...
        _load_and_update(functions, 'ctypes', ['CFUNCTYPE', 'PYFUNCTYPE'])
        _load_and_update(
            functions,
            'ctypes._endian',
            [
        ...
        if sys.version_info >= (3, 11):
            ...
            _load_and_update(
                classes,
                'ctypes._endian',
                ['_swapped_struct_meta', '_swapped_union_meta'],
            )
            _load_and_add(
                classes,
            ...
            _load_and_update(
                classes, 'typing', ['_AnyMeta', '_DeprecatedType']
            )
    ...
    if sys.implementation.name == 'pypy':
        ...
        _load_and_add(built_in_functions, 'parser', '_pickler')
        _load_and_update(
            built_in_functions, 'sys', ['__breakpointhook__', 'breakpointhook']
        )
        _load_and_add(built_in_functions, 'timeit', 'default_timer')
        _load_and_update(
            built_in_functions,
            'urllib.request',
            ['_get_proxies', '_get_proxy_settings'],
        )
        ...
        _load_and_add(built_in_functions, 'xxsubtype', 'bench')
        _load_and_add(
            built_in_functions, 'socketserver', '_ServerSelector._selector_cls'
        )
    ...
    if sys.implementation.name == 'pypy':
        ...
        _load_and_add(classes, '_ctypes.function', 'CFuncPtrType')
        _load_and_update(classes, '_ffi', ['CDLL', 'Field', 'WinDLL'])
        _load_and_update(
            classes,
            '_io',
            ['_BufferedIOBase', '_IOBase', '_RawIOBase', '_TextIOBase'],
        ...
        _load_and_update(
            built_in_functions,
            'collections',
            ['_count_elements', 'UserString.maketrans'],
        )
        _load_and_update(
            built_in_functions, 'ctypes', ['_check_HRESULT', '_dlopen']
        )
        _load_and_update(
            built_in_functions,
            'faulthandler',
        ...
        _load_and_update(
            ...
            ['Connection._read', 'Connection._write'],
        )
        _load_and_add(
            built_in_functions, 'multiprocessing.synchronize', 'sem_unlink'
        )
